...
/**
 * Filter a resume profile diff down to the selected categories only.
 * Recomputed counts, totalItems, and hasChanges match the kept categories so the
 * result can be passed directly to applyResumeProfileSync.
 * Categories: location, skills, education, experiences, projects, certifications.
 */
export function filterProfileDiff(diff, selectedCategories) {
  ...
  const toAdd = {
    location: selected.has("location") ? diff.toAdd?.location ?? null : null,
    skills: selected.has("skills") ? [...(diff.toAdd?.skills || [])] : [],
    education: selected.has("education") ? [...(diff.toAdd?.education || [])] : [],
    experiences: selected.has("experiences") ? [...(diff.toAdd?.experiences || [])] : [],
    projects: selected.has("projects") ? [...(diff.toAdd?.projects || [])] : [],
    certifications: selected.has("certifications") ? [...(diff.toAdd?.certifications || [])] : [],
  };

  const counts = {
    location: toAdd.location ? 1 : 0,
    skills: toAdd.skills.length,
    education: toAdd.education.length,
    experiences: toAdd.experiences.length,
    projects: toAdd.projects.length,
    certifications: toAdd.certifications.length,
  };
