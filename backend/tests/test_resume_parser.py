...
class TestStructuredExtraction(unittest.TestCase):
    def test_structured_output_validity_and_content(self):
        ...
        # Valid top-level structure. `summary` is an additive field added in
        # Resume Parsing 2.0, so the core set is a subset of the keys.
        self.assertTrue({
...
class TestResumeParsing20(unittest.TestCase):
    """Tests for the Resume Parsing 2.0 improvements: Unicode bullets, layout-
    aware structural parsing, multi-word skills, tool/company separation,
    project dates, summary, GPA, and generalised certifications."""
    ...
    def test_unicode_bullets_split_projects_without_blank_lines(self):
        text = """\
PROJECTS
Project Alpha | Python, TensorFlow, OpenCV
Feb 2026 \u2013 Mar 2026
- Built a model.
Project Beta | C# .NET, MySQL
Jul 2025 \u2013 Aug 2025
- Built a desktop app.
Project Gamma #2 | JSP, HTML, CSS
Aug 2025 \u2013 Oct 2025
- Built a web portal.
"""
        data = parse_resume(text).to_dict()
        names = [p["name"] for p in data["projects"]]
        # The 1->3 bug: all three projects must be separate entries.
        self.assertEqual(names, ["Project Alpha", "Project Beta", "Project Gamma #2"])
        first = data["projects"][0]
        self.assertIn("Python", first["technologies"])
        self.assertIn("TensorFlow", first["technologies"])
        self.assertEqual(first["dates"], "Feb 2026 - Mar 2026")

    def test_project_dates_and_technologies_present(self):
        text = """\
PROJECTS
Book Store Management System | C# .NET, MySQL, Visual Studio
Jul 2025 \u2013 Aug 2025
- Built a desktop application using C# (.NET).
"""
        data = parse_resume(text).to_dict()
        proj = data["projects"][0]
        self.assertEqual(proj["name"], "Book Store Management System")
        self.assertEqual(proj["dates"], "Jul 2025 - Aug 2025")
        self.assertIn("C#", proj["technologies"])
        self.assertIn(".NET", proj["technologies"])
        self.assertIn("application", proj["description"].lower())

    def test_multiple_skills_sections_kept_equal_count_to_project_count(self):
        # 3 projects each with their own tech, verifying structural splitting.
        text = """\
PROJECTS
...
"""
        ...
        self.assertEqual(data["projects"][1]["name"], "Booking System")
        self.assertEqual(data["projects"][2]["technologies"], ["Go"])

    def test_summary_field_captured(self):
        text = """\
PROFESSIONAL SUMMARY
IT undergraduate with strong foundations in Deep Learning and Image Processing.
SKILLS
...
"""
    ...
    def test_skills_extract_multiword_and_categories(self):
        text = """\
SKILLS
Programming: Python, Java, C++
Machine Learning & AI: Deep Learning, CNN, Image Processing, TensorFlow
...
"""
        ...
        skills = data["skills"]
        for expected in [
            "Python", "C++", "Deep Learning", "CNN", "Image Processing",
            "TensorFlow", "Data Structures and Algorithms", "Object-oriented Programming",
            "JSP", "Colab", "Git",
        ]:
            self.assertIn(expected, skills, f"missing {expected!r}")

    def test_tools_used_does_not_pollute_company(self):
        text = """\
EXPERIENCE
Software Developer Intern
Sep 2025 \u2013 Dec 2025
COEP Technological University (Remote)
Tools Used: Python, Excel, MATLAB
\u2013 Developed automation scripts.
"""
        data = parse_resume(text).to_dict()
        exp = data["experience"][0]
        self.assertEqual(exp["job_title"], "Software Developer Intern")
        self.assertEqual(exp["company"], "COEP Technological University")
        self.assertIn("Python", exp["tools_used"])
        self.assertIn("Excel", exp["tools_used"])
        self.assertEqual(exp["dates"], "Sep 2025 - Dec 2025")

    def test_unicode_apostrophe_institution_and_gpa(self):
        text = """\
EDUCATION
...
"""
        data = parse_resume(text).to_dict()
        edu = data["education"][0]
        self.assertIn("Kamalnayan Bajaj Institute of Engineering & Technology", edu["institution"])
        self.assertEqual(edu["gpa"], "8.98")
        self.assertEqual(edu["graduation_year"], "2027")
        self.assertEqual(edu["degree"], "B.Tech")

    def test_general_non_vendor_certifications(self):
        text = """\
AWARDS & CERTIFICATIONS
- Fundamentals of Object-Oriented Programming \u2013 NPTEL (2025): Scored 92%, ranked top 1%
- Programming in Python \u2013 NPTEL (2024)
- Boot-camp on Artificial Intelligence - C-DAC Pune (2025)
...
"""
        ...
        self.assertTrue(any("NPTEL" in c for c in certs))
        self.assertTrue(any("HackerRank" in c for c in certs))
        self.assertTrue(any("C-DAC" in c for c in certs))
        # Score/rank details after the colon must not be part of the cert name.
        self.assertFalse(any("Scored" in c for c in certs))

    def test_vendor_certs_still_recognized(self):
        text = """\
CERTIFICATIONS
AWS Certified Solutions Architect
...
"""
    ...
    def test_date_range_en_dash_is_not_a_bullet(self):
        from app.services.resume_parser import is_bullet_line, looks_like_date_line
        date_range = "Feb 2026 \u2013 Mar 2026"
        self.assertFalse(is_bullet_line(date_range))
        self.assertTrue(looks_like_date_line(date_range))
        self.assertTrue(is_bullet_line("- a real bullet"))
        self.assertTrue(is_bullet_line("\u2013 another bullet"))

    def test_page_number_filtered_from_project_description(self):
        text = """\
PROJECTS
Weather App
Python
...
"""
        data = parse_resume(text).to_dict()
        proj = data["projects"][0]
        self.assertEqual(proj["name"], "Weather App")
        self.assertEqual(proj["dates"], "Feb 2026 - Mar 2026")
        self.assertIn("weather", proj["description"].lower())
        self.assertNotIn("2", proj["description"])

    def test_layout_aware_extraction_preserves_visual_separation(self):
        # Two visually separate project titles share the same extracted text
        # layer; layout blocks must still let structural parsing separate them.
        from app.services.resume_parser import extract_pdf_layout
        pdf = os.path.join(self.tmpdir, "layout.pdf")
        body = """\
PROJECTS
Store Analytics App
Python, Pandas
...
"""
        ...
        doc.close()
        data = parse_resume(
            "\n".join(l.text for l in extract_pdf_layout(pdf))
        ).to_dict()
        names = [p.get("name") for p in data["projects"]]
        self.assertEqual(names, ["Store Analytics App", "Portfolio Site"])

    def test_parsed_data_has_additive_fields(self):
        from app.services.resume_parser import ParsedResumeData, parse_resume
        data = parse_resume(TEXT_RESUME).to_dict()
        core = {"basic_info", "education", "experience", "projects", "certifications", "skills"}
        self.assertTrue(core.issubset(set(data.keys())))
        self.assertIn("summary", data)
        # Additive fields on entries are optional and backward compatible.
        self.assertTrue(all(set(["job_title", "company"]).issubset(e) for e in data["experience"]))
        self.assertTrue(all("name" in p for p in data["projects"]))

    def test_heading_with_awards_and_certifications_variant(self):
        text = """\
AWARDS & CERTIFICATIONS
...
"""
        data = parse_resume(text).to_dict()
        self.assertTrue(data["certifications"])
        self.assertEqual(data["projects"][0]["name"], "Demo App")
